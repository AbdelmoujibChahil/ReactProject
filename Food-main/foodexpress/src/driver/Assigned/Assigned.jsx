...
const Assigned = () => {
  ...
  const mapBackendOrders = (commandes) => {
    return commandes.map((commande) => ({
      ...
      customerName: commande.adresse_livraison?.full_name || commande.user?.name || "N/A",
      customerPhone: commande.adresse_livraison?.phone || commande.user?.phone || "N/A",
      deliveryAddress: commande.adresse_livraison
        ? `${commande.adresse_livraison.street_address}, ${commande.adresse_livraison.city || ""}`
        : commande.user?.adress || "N/A",

      livreur: commande.livreur,
      plats: commande.plats?.map((plat) => ({
        id: plat.id,
  ...
  const getStatusColor = (status) => {
    // Backend uses: "on_delivery", "en attente", etc.
  ...
  const getFilteredOrders = () => {
    ...
    if (filter === "en attente")
      return assignedOrders.filter((o) => o.status?.toLowerCase() === "en attente");
        ...
        <div className="header-content">
          ...
          <div className="header-stats-inline">
            ...
            <div className="stat-item">
              ...
              <div className="stat-info">
                <span className="stat-value">
                  {assignedOrders.filter((o) => o.status?.toLowerCase() === "en attente").length}
                </span>
                <span className="stat-text">en attente</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="filter-section">
        <div className="filter-container">
          ...
          <div className="filter-pills">
            ...
            <button
              className={`filter-pill ${filter === "en attente" ? "active" : ""}`}
              onClick={() => setFilter("en attente")}
            >
              <span className="pill-dot en attente"></span>
              en attente ({assignedOrders.filter((o) => o.status?.toLowerCase() === "en attente").length})
            </button>
            <button
              className={`filter-pill ${filter === "in-transit" ? "active" : ""}`}
              onClick={() => setFilter("in-transit")}
            >
              <span className="pill-dot transit"></span>
              In Transit ({assignedOrders.filter((o) => 
                o.status?.toLowerCase() === "in transit" || 
                o.status?.toLowerCase() === "on_delivery"
              ).length})
            </button>
      ...
      <div className="assigned-content">
        {loading ? (
          ...
          <div className="empty-state">
            ...
            <p>
              You don't have any {filter !== "all" ? filter : ""} deliveries at the moment.
            </p>
          </div>
        ) : (
                ...
                <div className="order-card-body">
                  ...
                  <div className="info-section">
                    ...
                    <div className="info-item">
                      ...
                      <span className="info-value">{delivery.customerName}</span>
                    </div>
                    <div className="info-item">
                      <Phone size={16} />
                      <span className="info-value">{delivery.customerPhone}</span>
                    </div>
                  </div>
                  ...
                  {/* Order Details */}
                  {delivery.orderDetails && delivery.orderDetails.items?.length > 0 && (
                    <div className="info-section">
                      <h4>Order Details</h4>
                      ...
                      <div className="order-total">
                        <strong>Total:</strong>
                        <strong>${delivery.orderDetails.total.toFixed(2)}</strong>
                      </div>
                    </div>
                  )}
        ...
        <div className="modal-overlay" onClick={() => setSelectedOrder(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              {selectedOrder.orderDetails && selectedOrder.orderDetails.items?.length > 0 && (
                <div className="modal-section">
                  <h3>Order Items</h3>
